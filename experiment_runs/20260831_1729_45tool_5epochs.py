"""45-tool SBERT FT, 5 epochs. `upgraded` only. Does not train LEGR."""
...
RUN = ROOT / "experiment_runs" / "20260831_1729"
SEED = 42
EPOCHS = 5
...
def main() -> None:
    RUN.mkdir(parents=True, exist_ok=True)
    log("5-EPOCH 45-tool SBERT start")
    results = []
    ...
    lines = [
        "# 45-tool SBERT FT — 5 epochs",
        "",
        "| Variant | Recall@1 | Tool-Set F1 | Mean GED | best epoch | n_eval | Status |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for r in results:
        m = r.get("metrics") or {}
        ...
        lines.append(
            ...
            f"{c('mean_ged_error')} | {r.get('best_epoch', '')} | {r.get('n_eval', '')} | {r.get('status')} |"
        )
    (RUN / "RESULTS.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    log("5-EPOCH 45-tool SBERT COMPLETE")
